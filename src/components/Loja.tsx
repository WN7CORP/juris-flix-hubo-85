...
export const Loja = () => {
  ...
  return <div className="min-h-screen bg-background">
      ...
      {showIntro ? (/* Tela de Introdução Compacta */
          ...
          <div className="card-legal rounded-xl p-6 mb-6 text-center relative overflow-hidden animate-scale-in">
            ...
            <div className="relative z-10">
              ...
              <div className="bg-store-primary/10 border border-store-primary/30 rounded-lg p-4 mb-4 animate-premium-glow">
                ...
                <div className="bg-store-primary text-white px-4 py-2 rounded-lg inline-flex items-center gap-2 cursor-pointer transition-all duration-200 hover:scale-105 hover:shadow-lg active:scale-95" onClick={handleCopyCoupon}>
                  <code className="text-base font-mono font-bold">WN7PR10</code>
                  {copiedCoupon ? <Check className="h-4 w-4 animate-scale-in" /> : <Copy className="h-4 w-4" />}
                </div>
              ...
              {/* Botão Principal */}
              <button onClick={handleEnterStore} disabled={isLoading} className="btn-store px-6 py-3 rounded-xl text-base font-semibold inline-flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed animate-glow-pulse w-full justify-center">
                {isLoading ? <>
    ...
    <div className="w-full" style={{
      height: 'calc(100vh - 56px)'
          ...
          <iframe src="https://preview--barba-growth-journey-76.lovable.app/" className="w-full h-full border-0" title="Loja de Direito" sandbox="allow-same-origin allow-scripts allow-forms allow-popups allow-top-navigation" loading="lazy" />
        </div>)}
    </div>;
};